...
set CMAKE_OPTIONS=%CMAKE_OPTIONS% -DMEDFILE_ROOT_DIR=%MEDFILE_ROOT_DIR%
...
if NOT %ERRORLEVEL% == 0 (
    echo ERROR on msbuild %MAKE_OPTIONS% /p:Configuration=%PRODUCT_BUILD_TYPE% /p:Platform=x64 ALL_BUILD.vcxproj
    exit 2
)
...
if NOT %ERRORLEVEL% == 0 (
    echo ERROR on msbuild %MAKE_OPTIONS% /p:Configuration=%PRODUCT_BUILD_TYPE% /p:Platform=x64 INSTALL.vcxproj
    exit 3
)
